    ...
    <v-row>
      <v-col cols="9">
        <v-row>
          <v-col cols="6">
            <highchart
              :options="
                createAccLossConfig(
                  'accuracy',
                  'Accuracy against Train Set for: '
                )
              "
            >
            </highchart>
          ...
          <v-col cols="6">
            <highchart
              :options="
                createAccLossConfig('loss', 'Loss against Train Set for: ')
              "
            ></highchart>
          </v-col>
          <v-col cols="6">
          ...
          <v-col cols="6">
            <highchart
              :options="
                createAccLossConfig('val_loss', 'Loss against Test Set for: ')
              "
            ></highchart>
          </v-col>
        </v-row>
      ...
      <v-col cols="3">
        <v-list>
          ...
          <v-list-item v-for="item in genRunMetrics()" :key="item.time">
...
const triggerTraining = () => {
  fetch(
    `/api/train/models/${dataSet.value[0].name}/run/${dataSet.value[0].dataSet}`,
    {
      method: "POST",
      headers: {
...
const createAccLossConfig = (type, title) => {
  const series = data.value
    .filter((x) => x.dataSet == dataSet.value)
    .map((x) => {
      return {
